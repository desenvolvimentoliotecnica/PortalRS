...
export function UnidadeLotacaoAutocomplete({
  ...
  const [results, setResults] = useState<UnidadeLotacaoLookup[]>([]);
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      ...
      try {
        setLoading(true);
        const res = await apiFetch(`/api/unidades-lotacao/lookup?search=${encodeURIComponent(search)}`);
        if (res.ok) {
          const data = await res.json();
          const raw = Array.isArray(data) ? data : [];
          const mapped: UnidadeLotacaoLookup[] = raw.map((x: Record<string, unknown>) => ({
            id: String(x.id ?? x.Id ?? ""),
        ...
        console.error("Erro ao buscar unidades de lotação:", err);
        setResults([]);
      } finally {
        setLoading(false);
      }
