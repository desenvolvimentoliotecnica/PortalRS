...
export interface CategoriaSalarialLookup {
  id: string;
  code: string;
  description: string;
}

interface CategoriaSalarialAutocompleteProps {
  value: string | number | null;
  onChange: (code: string) => void;
  onSelectId?: (id: string) => void;
  /** Disparado ao escolher um item na lista (inclui descrição para o formulário). */
  onSelectItem?: (item: CategoriaSalarialLookup) => void;
  defaultLabel?: { code: string; description: string };
  placeholder?: string;
}

export function CategoriaSalarialAutocomplete({
  value,
  onChange,
  onSelectId,
  onSelectItem,
  defaultLabel,
  placeholder = "Selecione a categoria salarial...",
}: CategoriaSalarialAutocompleteProps) {
  const [search, setSearch] = useState("");
  const [results, setResults] = useState<CategoriaSalarialLookup[]>([]);
  const [loading, setLoading] = useState(false);
  const [open, setOpen] = useState(false);
  const [selected, setSelected] = useState<CategoriaSalarialLookup | null>(null);
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      ...
      try {
        setLoading(true);
        const res = await apiFetch(`/api/categorias-salariais/lookup?search=${encodeURIComponent(search)}`);
        if (res.ok) { const data = await res.json(); setResults(Array.isArray(data) ? data : []); setOpen(true); }
      } catch (err) { console.error("Erro ao buscar categorias salariais:", err); setResults([]); }
  ...
  useEffect(() => {
    if (value && !selected) {
      ...
      if (found) { setSelected(found); }
        ...
        setSelected({ id: "", code: strValue, description: `Categoria ${strValue}` });
      }
    }
  ...
  const handleSelect = (item: CategoriaSalarialLookup) => {
    setSelected(item);
    onChange(item.code);
    onSelectId?.(item.id);
    onSelectItem?.(item);
