...
export default function ExitInterviewPageClient() {
  ...
  useEffect(() => {
    fetch(`/api/public/exit-interview/${token}`)
      .then((r) => {
        if (r.status === 404) throw new Error("Link invalido ou nao encontrado.");
        if (!r.ok) throw new Error("Erro ao carregar formulario.");
  ...
  if (state.kind === "loading") {
    return <Layout><p className="text-gray-500 text-center">Carregando formulario...</p></Layout>;
  }
      ...
      <Layout>
        <div className="rounded-lg bg-yellow-50 border border-yellow-200 p-6 text-center">
          ...
          <p className="text-sm text-gray-500 mt-2">
            O prazo para responder a entrevista de saida encerrou.
            Entre em contato com o RH se precisar de mais informacoes.
          </p>
        </div>
      </Layout>
    );
    ...
    return (
      <Layout>
        <div className="rounded-lg bg-green-50 border border-green-200 p-6 text-center">
          <p className="text-green-700 font-semibold">Formulario ja preenchido.</p>
          <p className="text-sm text-gray-500 mt-2">
            Suas respostas ja foram registradas. Obrigado pela sua participacao!
          </p>
        </div>
      </Layout>
    );
      ...
      <Layout>
        <div className="rounded-lg bg-green-50 border border-green-200 p-8 text-center space-y-3">
          ...
          <p className="text-sm text-gray-500">
            Suas respostas sao confidenciais e nos ajudam a melhorar continuamente.
    ...
    <Layout>
      <div className="space-y-6">
        <div className="text-center space-y-1">
          <p className="text-sm text-gray-500">Ola, <span className="font-medium text-gray-700">{data.funcionarioNome}</span>!</p>
          <p className="text-sm text-gray-500">
            Suas respostas sao confidenciais e muito importantes para nos.
          </p>
        </div>
        ...
        {validationErrors.size > 0 && (
          <p className="text-center text-sm text-red-600">
            Preencha todas as perguntas obrigatorias antes de enviar.
          </p>
        )}

        <p className="text-center text-xs text-gray-400">
          Este link e valido por 30 dias a partir do envio. Suas respostas sao confidenciais.
        </p>
      </div>
    </Layout>
  );
...
function QuestionCard({ pergunta, index, resposta, hasError, onChange }: QuestionCardProps) {
  return (
    <div className={`rounded-lg border bg-white p-5 space-y-3 ${hasError ? "border-red-400" : "border-gray-200"}`}>
      ...
      {hasError && (
        <p className="text-xs text-red-600">Esta pergunta e obrigatoria.</p>
      )}
    </div>
  );
}
    ...
    <div className="min-h-screen bg-gray-50 flex items-start justify-center p-4 py-10">
      <div className="w-full max-w-lg">
        <div className="mb-6 text-center">
          <h1 className="text-xl font-bold text-gray-800">Entrevista de Saida</h1>
