...
export default function CentroCustoCadastroScreen() {
  ...
  const save = async () => {
    ...
    try {
      ...
      setEditOpen(false);
      await syncList();
    } catch { toast.error("Erro ao salvar centro de custo"); }
  ...
  const runImport = async () => {
    ...
    for (const row of importRows) {
      const payload = { code: row.code, description: row.description, manager: row.manager || null, notes: null, isActive: row.isActive };
      try {
        const existingId = existingMap.get(row.code.toLowerCase());
        if (existingId) { await fetchJson(`/api/centros-custo/${existingId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); updated++; }
        else { await fetchJson("/api/centros-custo", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); created++; }
      } catch { errors++; }
    }
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow key={item.id}>
                ...
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-1">
                    <Button variant="outline" size="icon-xs" title="Editar" onClick={() => { setDraft({ id: item.id, code: item.code, description: item.description, manager: item.manager || "", notes: item.notes || "", isActive: item.isActive }); setEditOpen(true); }}>
                      <Pencil />
                    </Button>
              ...
              <TableRow><TableCell colSpan={4} className="py-8 text-center text-muted-foreground">Nenhum centro de custo encontrado.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
            ...
            <div className="max-h-64 overflow-y-auto border rounded-md">
              <table className="w-full text-sm">
                ...
                <tbody>
                  ...
                  {importRows.length > 50 && <tr className="border-t"><td colSpan={4} className="px-3 py-2 text-center text-muted-foreground text-xs">… e mais {importRows.length - 50} registro(s)</td></tr>}
                </tbody>
              </table>
            </div>
          )}
