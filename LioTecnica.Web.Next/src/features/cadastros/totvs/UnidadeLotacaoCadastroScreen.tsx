...
export default function UnidadeLotacaoCadastroScreen() {
  ...
  const save = async () => {
    ...
    try {
      ...
      setEditOpen(false);
      await syncList();
    } catch { toast.error("Erro ao salvar unidade de lotação"); }
  ...
  const runImport = async () => {
    ...
    for (const row of importRows) {
      const payload = { code: row.code, description: row.description, location: row.location || null, manager: row.manager || null, notes: null, isActive: row.isActive };
      try {
        const existingId = existingMap.get(row.code.toLowerCase());
        if (existingId) { await fetchJson(`/api/unidades-lotacao/${existingId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); updated++; }
        else { await fetchJson("/api/unidades-lotacao", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); created++; }
      } catch { errors++; }
    }
  ...
  const exportTsv = () => {
    const csv = [["Código", "Descrição", "Localização", "Responsável", "Status"].join("\t"),
      ...rows.map((x) => [x.code, x.description, x.location || "", x.manager || "", x.isActive ? "Ativo" : "Inativo"].join("\t"))
    ].join("\n");
    ...
    <section className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <div className="text-muted-foreground text-sm">Cadastro de unidades de lotação.</div>
        </div>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow key={item.id}>
                ...
                <TableCell className="text-muted-foreground">{item.manager || "—"}</TableCell>
                <TableCell>{statusBadge(item.isActive)}</TableCell>
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-1">
                    <Button variant="outline" size="icon-xs" title="Editar" onClick={() => { setDraft({ id: item.id, code: item.code, description: item.description, location: item.location || "", manager: item.manager || "", notes: item.notes || "", isActive: item.isActive }); setEditOpen(true); }}>
                      <Pencil />
                    </Button>
              ...
              <TableRow><TableCell colSpan={5} className="py-8 text-center text-muted-foreground">Nenhuma unidade de lotação encontrada.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Preencha os dados da unidade de lotação.</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Excluir a unidade <strong>"{deleteTarget?.description}"</strong>? Esta ação não pode ser desfeita.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteTarget(null)}>Cancelar</Button>
            <Button variant="destructive" onClick={async () => { if (!deleteTarget) return; try { await fetchJson(`/api/unidades-lotacao/${deleteTarget.id}`, { method: "DELETE" }); toast.success("Unidade removida"); setDeleteTarget(null); await syncList(); } catch { toast.error("Erro ao remover"); } }}>Excluir</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </section>
