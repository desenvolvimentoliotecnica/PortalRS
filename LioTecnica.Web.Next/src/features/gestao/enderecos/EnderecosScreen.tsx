...
export default function EnderecosScreen() {
  ...
  const [newCidadeSuggestions, setNewCidadeSuggestions] = useState<string[]>([]);
  const [newCidadeFocused, setNewCidadeFocused] = useState(false);
  ...
  async function lookupCep(raw: string) {
    ...
    finally { setNewCepLoading(false); }
  }

  const cidadeDebounceRef = React.useRef<ReturnType<typeof setTimeout> | null>(null);
  function onCidadeChange(v: string) {
    setNewCidade(v);
    setNewCidadeSuggestions([]);
    if (cidadeDebounceRef.current) clearTimeout(cidadeDebounceRef.current);
    if (v.trim().length < 2) return;
    cidadeDebounceRef.current = setTimeout(async () => {
      try {
        const res = await fetch(`https://servicodados.ibge.gov.br/api/v1/localidades/municipios?nome=${encodeURIComponent(v.trim())}&orderBy=nome`);
        if (!res.ok) return;
        const data = await res.json() as { nome: string }[];
        setNewCidadeSuggestions(data.slice(0, 8).map((m) => m.nome));
      } catch { /* ignore */ }
    }, 300);
  }
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => {
            apiFetch(`${API}/export`).then((r) => r.blob()).then((blob) => { const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "enderecos.csv"; a.click(); }).catch(() => toast.error("Falha ao exportar."));
          }}>
      ...
      {/* New request dialog */}
      <Dialog open={newOpen} onOpenChange={(o) => { if (!o) { setNewOpen(false); setNewFuncionario(null); setNewCep(""); setNewLogradouro(""); setNewNumero(""); setNewBairro(""); setNewComplemento(""); setNewCidade(""); setNewCidadeSuggestions([]); setNewUf("SP"); setNewObs(""); } }}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Nova solicitação de endereço</DialogTitle>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <div className="relative mt-1">
                  <input
                    className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-1 focus:ring-ring pr-8"
                    placeholder="00000-000"
                    value={newCep}
                    onChange={(e) => { setNewCep(e.target.value); void lookupCep(e.target.value); }}
                  />
                  {newCepLoading && <div className="absolute right-2 top-1/2 -translate-y-1/2 h-4 w-4 animate-spin rounded-full border-2 border-t-transparent border-primary" />}
                </div>
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground uppercase">UF</label>
                <select className="mt-1 h-9 w-full rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-1 focus:ring-ring" value={newUf} onChange={(e) => setNewUf(e.target.value)}>
            ...
            <div>
              ...
              <input className="mt-1 h-9 w-full rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-1 focus:ring-ring" placeholder="Rua, avenida…" value={newLogradouro} onChange={(e) => setNewLogradouro(e.target.value)} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-medium text-muted-foreground uppercase">Número</label>
                <input className="mt-1 h-9 w-full rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-1 focus:ring-ring" placeholder="123" value={newNumero} onChange={(e) => setNewNumero(e.target.value)} />
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground uppercase">Complemento</label>
                <input className="mt-1 h-9 w-full rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-1 focus:ring-ring" placeholder="Apto, bloco…" value={newComplemento} onChange={(e) => setNewComplemento(e.target.value)} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-medium text-muted-foreground uppercase">Bairro</label>
                <input className="mt-1 h-9 w-full rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-1 focus:ring-ring" placeholder="Bairro" value={newBairro} onChange={(e) => setNewBairro(e.target.value)} />
              </div>
              <div className="relative">
                <label className="text-xs font-medium text-muted-foreground uppercase">Cidade *</label>
                <input
                  ...
                  onFocus={() => setNewCidadeFocused(true)}
                  onBlur={() => setTimeout(() => setNewCidadeFocused(false), 150)}
                  autoComplete="off"
                />
                {newCidadeFocused && newCidadeSuggestions.length > 0 && (
                  <ul className="absolute z-50 mt-1 w-full rounded-md border border-border bg-popover shadow-md">
                    {newCidadeSuggestions.map((c) => (
                      <li key={c}
                        className="cursor-pointer px-3 py-1.5 text-sm hover:bg-muted"
                        onMouseDown={() => { setNewCidade(c); setNewCidadeSuggestions([]); }}>
                        {c}
                      </li>
                    ))}
                  </ul>
          ...
          <DialogFooter>
            ...
            <Button disabled={newSaving || !newLogradouro || !newCep || !newCidade} onClick={async () => {
              setNewSaving(true);
              try {
                await fetchJson(API, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ funcionarioId: newFuncionario?.id ?? null, cep: newCep, logradouro: newLogradouro, numero: newNumero || null, bairro: newBairro || null, complemento: newComplemento || null, cidade: newCidade, uf: newUf, observacoes: newObs || null }) });
