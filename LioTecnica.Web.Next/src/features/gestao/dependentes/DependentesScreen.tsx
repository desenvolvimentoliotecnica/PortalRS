...
import {
  ...
  FileText, Users, Eye, UserCheck, CalendarDays, Download, Plus,
} from "lucide-react";
...
const API = "/api/colaborador/solicitacoes-dependente";
...
const TIPO_SOL: Record<number, string> = { 0: "Inclusão", 1: "Alteração", 2: "Exclusão" };
const PARENTESCO: Record<number, string> = { 0: "Cônjuge", 1: "Filho(a)", 2: "Pai", 3: "Mãe", 4: "Outro" };
...
export default function DependentesScreen() {
  ...
  const isRH = (me?.roles ?? []).some((r: string) => r.toLowerCase() === "rh");
  const myFuncionarioId = (me as { funcionarioId?: string } | null)?.funcionarioId;

  const [loading, setLoading] = useState(true);
  const [rows, setRows] = useState<DependenteGridRow[]>([]);
  ...
  const [detail, setDetail] = useState<DependenteDetail | null>(null);
  ...
  const [newTipo, setNewTipo] = useState(0);
  const [newNome, setNewNome] = useState("");
  const [newParentesco, setNewParentesco] = useState(0);
  const [newCpf, setNewCpf] = useState("");
  const [newNascimento, setNewNascimento] = useState("");
  const [newPcd, setNewPcd] = useState(false);
  const [newDepIr, setNewDepIr] = useState(false);
  const [newObs, setNewObs] = useState("");
  const [newSaving, setNewSaving] = useState(false);

  const load = useCallback(async () => {
    const data = await fetchJson<DependenteGridRow[]>(API);
  ...
  const kpis = useMemo(() => ({
    total: rows.length,
    pendentes: rows.filter((r) => isPending(r.status)).length,
    aprovadas: rows.filter((r) => r.status === 2).length,
    reprovadas: rows.filter((r) => r.status === 3).length,
  }), [rows]);

  async function openDetail(row: DependenteGridRow) {
    setDetailOpen(true); setDetailLoading(true); setApprovalObs("");
    try { setDetail(await fetchJson<DependenteDetail>(`${API}/${row.id}`)); }
  ...
  async function assumirEtapa(id: string) {
    try {
      ...
      await load();
    } catch (e) { toast.error(`Falha ao assumir: ${e instanceof Error ? e.message : "erro"}`); }
  }
  ...
  async function quickApprove(id: string) {
    try {
      ...
      await load();
    } catch (e) { toast.error(`Falha ao aprovar: ${e instanceof Error ? e.message : "erro"}`); }
  }
  ...
  async function confirmReject() {
    ...
    try {
      ...
      setRejectTarget(null); setRejectObs("");
      await load();
    } catch (e) { toast.error(`Falha ao reprovar: ${e instanceof Error ? e.message : "erro"}`); }
  }
  ...
  async function confirmChanges() {
    ...
    try {
      ...
      setChangesTarget(null); setChangesObs("");
      await load();
    } catch (e) { toast.error(`Falha: ${e instanceof Error ? e.message : "erro"}`); }
  }

  /* Determine if current user can approve the pending step in detail dialog */
  const { canApprove, isQueueStep } = useMemo(() => {
    if (!detail || !me) return { canApprove: false, isQueueStep: false };
    if (!isPending(detail.status)) return { canApprove: false, isQueueStep: false };
    if (isAdmin || isRH) return { canApprove: true, isQueueStep: false };
    return { canApprove: false, isQueueStep: false };
  }, [detail, me, isAdmin, isRH]);

  return (
    <section className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-muted-foreground text-sm">Inclusão, alteração e exclusão de dependentes</p>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => {
            apiFetch(`${API}/export`).then((r) => r.blob()).then((blob) => { const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "dependentes.csv"; a.click(); }).catch(() => toast.error("Falha ao exportar."));
          }}>
      ...
      <div className="card-soft rounded-xl border border-border/40 bg-card/60 p-4 backdrop-blur">
        ...
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
          <div>
            <div className="font-semibold">Solicitações de dependentes</div>
          ...
          <div className="relative min-w-[220px] flex-1 max-w-sm">
            ...
            <Input className="pl-9" placeholder="Buscar colaborador ou dependente…" value={q} onChange={(e) => setQ(e.target.value)} />
          </div>
        </div>
        {/* Row 2: status chips */}
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow key={r.id} className="hover:bg-muted/40">
                ...
                <TableCell className="text-sm">{PARENTESCO[r.parentesco] ?? "—"}</TableCell>
                <TableCell className="text-sm">{TIPO_SOL[r.tipoSolicitacao] ?? "—"}</TableCell>
                <TableCell><StatusBadge status={r.status} /></TableCell>
                <TableCell>
                  {isPending(r.status) && r.etapaPendenteLabel ? (
                ...
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-1" onClick={(e) => e.stopPropagation()}>
                    {isPending(r.status) && r.etapaPendenteCanAssume ? (
                      <>
                        ...
                        {r.etapaPendenteIsQueue && (
                          <Button size="sm" variant="outline" className="h-7 px-2 text-xs text-blue-700 border-blue-300 hover:bg-blue-50"
                            onClick={() => void assumirEtapa(r.id)}>
              ...
              <TableRow><TableCell colSpan={8} className="py-8 text-center text-muted-foreground">Nenhuma solicitação encontrada.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </div>
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Detalhes — Dependente</DialogTitle>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3 text-sm">
                {([
                  ...
                  ["Tipo",          TIPO_SOL[detail.tipoSolicitacao] ?? "—"],
                  ["Nome Completo", detail.nomeCompleto],
                  ["Parentesco",    PARENTESCO[detail.parentesco] ?? "—"],
                  ["CPF",           detail.cpf ?? "—"],
                  ["Nasc.",         fmtDate(detail.dataNascimento)],
                  ["PCD",           detail.isPcd ? "Sim" : "Não"],
                  ["Dep. IR",       detail.dependenteIR ? "Sim" : "Não"],
                  ...
                  <div className="flex gap-2">
                    ...
                    {isQueueStep && (
                      <Button size="sm" variant="outline" className="text-blue-600 border-blue-300 hover:bg-blue-50"
                        onClick={() => void assumirEtapa(detail.id)}>
      ...
      {/* New request dialog */}
      <Dialog open={newOpen} onOpenChange={(o) => { if (!o) { setNewOpen(false); setNewFuncionario(null); setNewTipo(0); setNewNome(""); setNewParentesco(0); setNewCpf(""); setNewNascimento(""); setNewPcd(false); setNewDepIr(false); setNewObs(""); } }}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select value={newTipo} onChange={(e) => setNewTipo(Number(e.target.value))} className="mt-1 w-full h-9 rounded-md border border-input bg-background px-3 text-sm">
              ...
              <div>
                <label className="text-xs font-medium text-muted-foreground uppercase">Parentesco</label>
                <select value={newParentesco} onChange={(e) => setNewParentesco(Number(e.target.value))} className="mt-1 w-full h-9 rounded-md border border-input bg-background px-3 text-sm">
            ...
            <div>
              ...
              <input type="text" value={newNome} onChange={(e) => setNewNome(e.target.value)} className="mt-1 w-full h-9 rounded-md border border-input bg-background px-3 text-sm" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-medium text-muted-foreground uppercase">CPF</label>
                <input type="text" value={newCpf} onChange={(e) => setNewCpf(e.target.value)} placeholder="000.000.000-00" className="mt-1 w-full h-9 rounded-md border border-input bg-background px-3 text-sm placeholder:text-muted-foreground" />
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground uppercase">Data de nascimento</label>
                <input type="date" value={newNascimento} onChange={(e) => setNewNascimento(e.target.value)} className="mt-1 w-full h-9 rounded-md border border-input bg-background px-3 text-sm" />
              </div>
            </div>
            <div className="flex gap-4">
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input type="checkbox" checked={newPcd} onChange={(e) => setNewPcd(e.target.checked)} className="rounded" /> É PCD
              </label>
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input type="checkbox" checked={newDepIr} onChange={(e) => setNewDepIr(e.target.checked)} className="rounded" /> Dependente IR
              </label>
            </div>
            <div>
          ...
          <DialogFooter>
            ...
            <Button disabled={newSaving || !newNome || !newNascimento} onClick={async () => {
              setNewSaving(true);
              try {
                await fetchJson(API, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ funcionarioId: newFuncionario?.id ?? null, tipoSolicitacao: newTipo, nomeCompleto: newNome, parentesco: newParentesco, cpf: newCpf || null, dataNascimento: newNascimento, isPcd: newPcd, dependenteIR: newDepIr, observacoes: newObs || null }) });
