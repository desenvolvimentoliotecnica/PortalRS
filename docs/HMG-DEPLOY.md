...
Fluxo: **push/merge em `HML`** (por exemplo merge de um PR vindo de `DEV`) dispara [`.github/workflows/deploy-hmg.yml`](../.github/workflows/deploy-hmg.yml):
...
O fluxo regular de HML deve usar PR para `HML` + GitHub Actions.
