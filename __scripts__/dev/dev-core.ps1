...
Write-Host "▶ Aguardando API em http://localhost:5056/health (máx. 90s)..." -ForegroundColor DarkGray
$max = 90
$ready = $false
while ($max -gt 0) {
    try {
...
if (-not (Test-Path "$nextDir\node_modules")) {
    Write-Host "▶ Instalando dependências do Next.js..." -ForegroundColor DarkGray
