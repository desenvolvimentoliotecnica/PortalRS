(function () {
    "use strict";
    const API = "/Feedback/_api/plans/my";
    ...
    async function load() {
        ...
        if (plansEmpty) plansEmpty.classList.add("d-none");
        try {
            const data = await apiGet(API + "?page=1&pageSize=50");
            const items = data.items || [];
            items.forEach(p => { if (plansList) plansList.insertAdjacentHTML("beforeend", renderPlan(p)); });
            if (items.length === 0 && plansEmpty) plansEmpty.classList.remove("d-none");
        } catch (e) {
            console.error(e);
            if (plansList) plansList.innerHTML = "<div class=\"text-danger\">Erro ao carregar.</div>";
        }
    }
