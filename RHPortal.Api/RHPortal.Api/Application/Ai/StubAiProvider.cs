...
public sealed class StubAiProvider(ILogger<StubAiProvider> logger) : IAiProvider
{
    ...
    {
        logger.LogWarning(
            "StubAiProvider ativo — retornando resposta falsa (provider={Provider}, model={Model}). " +
            "Configure um provider real (OpenAI/Azure) para respostas reais.",
