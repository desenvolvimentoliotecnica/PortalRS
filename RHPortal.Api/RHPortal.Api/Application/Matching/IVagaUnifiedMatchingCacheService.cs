...
public interface IVagaUnifiedMatchingCacheService
{
    ...
    /// </summary>
    Task<VagaUnifiedMatchingRankingSnapshot> GetOrStartAsync(Guid vagaId, int take = 20, CancellationToken ct = default);

    /// <summary>
    /// Marca a vaga como pendente e dispara recálculo em background (idempotente).
    /// Usado ao editar filtros de matching.
    /// </summary>
    Task<VagaUnifiedMatchingRankingSnapshot> InvalidateAndStartAsync(Guid vagaId, int take = 20, CancellationToken ct = default);
}
